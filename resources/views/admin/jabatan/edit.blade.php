...
<div class="h-screen flex flex-row flex-wrap">
    ...
    <div class="flex-grow bg-white">
        ...
        <!-- Container untuk form edit data jabatan -->
        <div class="w-full h-screen min-w-max p-5 overflow-y-auto custom-scrollbar">
            <!-- Menampilkan breadcrumb -->
            @include('layout.breadcrumb2')
            <form class="px-10 py-10 min-w-full bg-white grid grid-cols-4 gap-x-20 gap-y-2 outline-none outline-4 outline-gray-700 rounded-xl" action="{{ url('admin/jabatan/' . $jabatan->id_level) }}" method="POST">
                ...
                <select name="kode_level" id="kode_level" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-gray-500 focus:border-gray-500 block col-span-4 p-2.5" placeholder="Masukkan Kode Level">
                    <option value="">Pilih Kode Jabatan</option>
                    <option value="ADM" @if($jabatan->kode_level == 'ADM') selected @endif>ADM (Admin)</option>
                    <option value="RW" @if($jabatan->kode_level == 'RW') selected @endif>RW</option>
                    <option value="RT" @if($jabatan->kode_level == 'RT') selected @endif>RT</option>
                </select>
